...
func (p *customermodel) Create(customer entities.Customer) bool {
	...
	result, err := p.conn.Exec("insert into customer (nama_lengkap, no_hp, merk, alamat) values(?,?,?,?)",
		customer.NamaLengkap, customer.NomorHp, customer.Merk, customer.Alamat)
...
func (p *customermodel) Update(customer entities.Customer) error {
	...
	_, err := p.conn.Exec(
		"update customer set nama_lengkap = ?, no_hp = ?, merk = ?, alamat = ? where id = ?",
		customer.NamaLengkap, customer.NomorHp, customer.Merk, customer.Alamat, customer.Id)
